...
namespace VirtualVoid.Net
{
    ...
    public class Message
    {
        ...
        {
            return $"ReadableLength: {ReadableLength}\nUnreadLength: {UnreadLength}\nWrittenLength: {WrittenLength}\nUnwrittenLength: {UnwrittenLength}";
        }
        ...
        private Message(ushort maxSize = SteamManager.PUBLIC_MESSAGE_BUFFER_SIZE)
        {
        ...
        /// <summary>Retrieves a <see langword="byte"/> from the message, without moving the readPos.</summary>
        /// <returns>The <see langword="byte"/> that was retrieved.</returns>
        ...
        public Message Add<T>(T str) where T : INetworkMessage, new()
        {
            ...
            {
                Debug.LogError($"Error adding {typeof(T).Name} to message! " + ex);
            }
            return this;
        }
        ...
        public Message Add(Client client)
        {
            if (client == null)
            {
                Debug.LogWarning("Tried to add client to message, but it was null!");
                return this;
            }

            if (!client.Authenticated)
            {
                Debug.LogWarning("Tried to add unauthenticated client to message!");
                return this;
            }    

            if (client.clientID == byte.MaxValue)
            {
                Debug.LogWarning("Adding client that has a clientID of 255, may be fine but likely an error occurred!");
            }
        ...
        {
            byte clientID = GetByte();

            return SteamManager.GetClient(clientID);

            //if (!SteamManager.clientIDToSteamID.TryGetValue(clientID, out SteamId id))
            //{
            //    Debug.LogWarning($"Tried to get client with ID of {clientID}, but no client with that ID exists!");
            //    return null;
            //}

            //if (!SteamManager.clients.TryGetValue(id, out Client client))
            //{
            //    Debug.LogWarning($"Tried to get client with SteamID {id} ({id.SteamName()}), but the clients dictionary does not contain that id!");
            //    return null;
            //}
        ...
        public T GetClient<T>() where T : Client
        {
            byte clientID = GetByte();

            return SteamManager.GetClient<T>(clientID);
        }

        public bool TryGetClient<T>(out T outClient) where T : Client
        {
            byte clientID = GetByte();

            return SteamManager.TryGetClient(clientID, out outClient);
        }
        ...
        //public Message HandlerToSender(SendType sendType, ushort id)
        //{
        //
        //}
        //
        //internal Message InternalHandlerToSender(SendType sendType, ushort id)
        //{
        //
        //}
    }
